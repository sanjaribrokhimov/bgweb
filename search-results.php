...
class SearchResultsLoader {
    ...
    async loadSearchResults() {
        ...
        try {
            const url = `http://144.126.128.67:8888/api/ads/search?q=${encodeURIComponent(this.query)}&type=${this.type}&page=${this.page}&limit=${this.limit}`;
    ...
    async showDetails(id, type) {
        try {
            ...
            const response = await fetch(`http://144.126.128.67:8888/api/ads/details/${type}/${id}`);
            if (!response.ok) {
    ...
    if (confirmYes) {
        confirmYes.addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await fetch('http://144.126.128.67:8888/api/notifications/accept', {
                    method: 'POST',
                    headers: {
