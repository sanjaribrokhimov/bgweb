...
class SearchResultsLoader {
    ...
    async loadSearchResults() {
        ...
        try {
            const url = `http://bgweb.nurali.uz/api/ads/search?q=${encodeURIComponent(this.query)}&type=${this.type}&page=${this.page}&limit=${this.limit}`;
    ...
    async showDetails(id, type) {
        try {
            ...
            const response = await fetch(`http://bgweb.nurali.uz/api/ads/details/${type}/${id}`);
            if (!response.ok) {
